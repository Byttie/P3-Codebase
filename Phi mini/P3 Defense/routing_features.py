...
def _top1_freq(payload, L, E):
    ...
    # ZAYA saved "top1_expert_histogram"; LFM2/Phi/Marco saved "topk_expert_histogram"
    # (top-K selection counts). Both are per-(layer,expert) frequency features and
    # are used identically here; accept whichever the payload has.
    hist = payload.get("top1_expert_histogram")
    if hist is None:
        hist = payload.get("topk_expert_histogram")
    if hist is None:
        raise KeyError("payload has neither top1_expert_histogram nor topk_expert_histogram")
    out = torch.zeros(L, E)
